  ...
  return (
    <div className="animate-fadeUpFast">
      ...
      <p className="mt-0.5 text-sm text-slate2">
        Marketplace de empresas autorizadas. Suscríbete y empieza a referir.
      </p>
      ...
      <div className="mt-5 grid grid-cols-1 gap-3.5 md:grid-cols-2">
        {empresas.map((e) => {
          ...
          return (
            <div
              key={e.id}
              className="relative rounded-[20px] border bg-white p-5 transition-all hover:-translate-y-0.5"
              style={{
            ...
            >
              ...
              <div className="mt-3 text-[17px] font-bold">{e.nombre}</div>
              <div className="mt-[3px] text-[13px] text-slate2">{e.descripcion ?? ''}</div>

              <div className="mt-3.5 flex flex-col gap-[7px] rounded-xl bg-surface p-3">
                <div className="flex justify-between text-[13px]">
                  <span className="text-slate3">Comisión</span>
              ...
              {on && oferta?.capacitacion !== 'ninguna' && (
                <div className="mt-2.5 animate-floatIn rounded-[10px] bg-[rgba(16,185,129,.08)] px-3 py-[9px] text-xs text-slate2">
                  📚 Capacitación requerida: <b>{CAPACITACION_LABELS[oferta.capacitacion]}</b> — la
                  encuentras en la pestaña Capacitación.
                </div>
              )}
            </div>
          );
